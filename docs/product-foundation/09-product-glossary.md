...
## Objective
Define Perch's official product vocabulary so that product, design, engineering, marketing, support, and documentation use the same language and do not invent competing meanings over time.
...
| --- | --- | --- |
| Perch | The product and brand: a personal browser dashboard for the new tab. | Discagem, speed dial app |
| Browser Home | The conceptual space Perch creates inside the browser. | New tab replacement, start page |
| Personal Browser Dashboard | The category Perch intends to define. | Bookmark manager, productivity dashboard |
| Dashboard | The user's main Perch surface containing cards, folders, widgets, and personal setup. | Homepage, panel |
| Home | The user's default starting place in Perch. | Start screen, initial page |
...
| Favorite | A browser-saved link imported into or represented by Perch. | Bookmark, shortcut |
...
| Sync | The paid continuity capability that keeps a user's Perch space consistent across devices. | Backup, cloud save |
...
| Continuity | The experience of the same Perch space across devices and time, enabled by sync. | Mirroring, syncing |
...
| Assistant Layer | The role of AI in Perch: it strengthens the product; it never becomes the product. | AI mode, AI feature |
...
- **"Discagem"** was the original project name, but **Perch** is now the canonical product and brand name.
- **"Speed dial"** describes a legacy pattern, not the category Perch wants to own. Never used as a self-description.
...
- **"Minimalism"** is an aesthetic; **Calm** is a felt quality. Perch pursues calm, which may or may not look minimalist in a given surface.
- **"Customization"** is a capability; **Ownership** is a feeling. Perch offers customization in service of ownership, not as an end in itself.
- **"Polish"** implies something added at the end; **Craft** is embedded throughout. Avoid "polish" when describing Perch's approach — it misrepresents when quality enters the product.
- **"Personalization"** refers to making something personal; **Ownership** refers to it being the user's. Perch prioritizes ownership; personalization is one means toward it, not the goal.