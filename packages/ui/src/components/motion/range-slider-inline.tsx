...
import { SPRING_GLIDE } from "@klice-start/ui/lib/ease";
import {
	type SliderOptions,
	snapSliderValue,
	useSlider,
} from "@klice-start/ui/lib/hooks/use-slider";
import {
	capturePointer,
	releasePointer,
	TOUCH_GESTURE_CLASS,
} from "@klice-start/ui/lib/touch";
...
function mapBetweenStops(
	...
	if (stops.length === 0) return point;
	const upperIndex = stops.findIndex((stop) => stop[from] >= point);
	const upper =
		stops[upperIndex < 0 ? stops.length - 1 : upperIndex] ??
		stops[stops.length - 1];
	const lower = stops[Math.max(0, upperIndex - 1)] ?? stops[0];
	if (!lower || !upper) return point;
...
export function InlineSlider({
		...
		const values = [
			...new Set(
				Array.from({ length: STOP_COUNT }, (_, index) =>
					snapSliderValue(
						min + (index / (STOP_COUNT - 1)) * (max - min),
						min,
						max,
						step,
					),
		...
		return values.map((value, index) => ({
			...
			x:
				values.length === 1
					? HANDLE_START
					: HANDLE_START +
						(index / (values.length - 1)) * (endX - HANDLE_START),
		}));
	}, [min, max, step, endX]);
	...
	const labelBounds = {
		start: TEXT_INSET,
		end: TEXT_INSET + geometry.labelWidth,
	};
	const readoutBounds = {
	...
	const ticks = showTicks
		? stops
				...
				.filter(
					(x) =>
						!overlapsText(x, labelBounds) && !overlapsText(x, readoutBounds),
				)
		: [];
	...
	return (
		<div
			...
			onPointerDown={(event) => {
				...
				const offset =
					Math.abs(pointerX - thumbX - 2) <= 12 ? pointerX - thumbX : 2;
				gesture.current = {
					id: event.pointerId,
					left: rect.left,
					offset,
			...
			onPointerMove={(event) => {
				const active = gesture.current;
				if (!active || active.id !== event.pointerId || options.disabled)
					return;
				const x = Math.min(
					endX,
		...
		>
			...
			>
				<span
					ref={labelRef}
					className="absolute top-1/2 left-5 max-w-[40%] -translate-y-1/2 truncate font-medium text-sm leading-5"
				>
					{label}
				</span>
				<span
					ref={readoutRef}
					className="absolute top-1/2 right-5 max-w-[40%] -translate-y-1/2 truncate font-semibold text-[13px] tabular-nums leading-[18px] tracking-tight"
				>
					{format(current)}
				</span>
				{ticks.map((left) => (
			...
			<button
				...
				onKeyDown={(event) => {
					...
					const next = {
						ArrowRight:
							stops.find((stop) => stop.value > current)?.value ?? max,
						ArrowUp: stops.find((stop) => stop.value > current)?.value ?? max,
						ArrowLeft:
							stops.findLast((stop) => stop.value < current)?.value ?? min,
						ArrowDown:
							stops.findLast((stop) => stop.value < current)?.value ?? min,
						Home: min,
						End: max,
